...
- `SSH_CONTROL_PERSIST_SECONDS` may override the persistence bound from 1 to 3600 seconds, `SSH_CONNECT_TIMEOUT_SECONDS` may override the connection timeout from 1 to 300 seconds, and `SSH_MASTER_RETRY_BACKOFF_SECONDS` may override failed-target backoff from 1 to 300 seconds. Defaults are `30`, `10`, and `5`. The connection timeout limits establishment only; there is no additional timeout after the remote command starts. Prepare keys/agent access and `known_hosts` first, because LHC will not prompt for a password or host-key confirmation.
